...
```
demo        <- Root directory. Both monorepo and polyrepo are supported.
│
├── demo       <- Contains python code, notebooks and ML resources related to one ML project. 
│   │
...
* An automated integration test is provided (in `.github/workflows/demo-run-tests.yml`) that executes a multi task run on Databricks involving the feature engineering and model training notebooks.
