...
ML Resource Configurations in this directory:
 - model workflow (`demo/resources/model-workflow-resource.yml`)
 - batch inference workflow (`demo/resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`demo/resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`demo/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`demo/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `demo/databricks.yml` and `demo/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-demo-model     | /dev-demo-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-demo-model | /staging-demo-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-demo-model    | /prod-demo-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-demo-model    | /test-demo-experiment    |
...
Following this, update the batch_inference_job base parameters in `demo/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `demo/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-demo-batch-inference-job`.
